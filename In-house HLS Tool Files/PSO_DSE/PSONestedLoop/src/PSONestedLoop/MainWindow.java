...
public class MainWindow extends javax.swing.JFrame {
 ...
 //  chooser.setCurrentDirectory(new java.io.File("."));
        ...
        if(gc.nestedloop){
           psn.preProcessing();
            setofdata.setEnabled(false);
        }
       else if(gc.CDFG){
              gc.UFList= ps.preProcessing(gc.setofdata);
              setofdata.setEnabled(false);
        }
        else{
    ...
    private void OKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_OKActionPerformed
       ...
       System.out.println("max-  "+ gc.max[j]);  
       }   
        ...
        for(int j=0;j<(gc.size);j++){
            ...
            total=total*gc.max[j];
                ...
                total *= gc.loopcount.get(j-gc.noofresources);  
        }
        }
       else if(gc.CDFG){
        for(int j=0;j<(gc.size);j++)
        ...
        if(powerNtexe.isSelected()){
         ...
         if(gc.nestedloop){
             PSOProcessNestedLoop ppnl=new PSOProcessNestedLoop();
             ppnl.execute();
         }
       else  if(gc.CDFG){
             PSOProcessLoop pp=new PSOProcessLoop();
                pp.execute();
         }
         else{
      ...
      System.out.println("Convergence time- "+contime+"\nexploration time- "+elapsedTime);
       ...
       if(gc.nestedloop){
         if(gc.power_texe){
            power=psn.calculatePower(gc.result_comp);
            exeTime=psn.calculateFinalTex(gc.result_comp); 
            gc.output=gc.output+"\npower= "+power+"mw\nExecution Time= "+exeTime+"us\n";
          }
          if(gc.area_texe){
               area=psn.calculateArea(gc.result_comp);
              exeTime=psn.calculateFinalTex(gc.result_comp);
          ...
          if(gc.power_texe){
            power=ps.calculatePower(gc.result_comp);
            exeTime=ps.calculateTex(gc.result_comp); 
            gc.output=gc.output+"\npower= "+power+"mw\nExecution Time= "+exeTime+"us\n";
          }
          if(gc.area_texe){
               area=ps.calculateArea(gc.result_comp);
              exeTime=ps.calculateTex(gc.result_comp);
              gc.output=gc.output+"\nArea= "+area+"#transistor\nExecution Time= "+exeTime+"us\n";
          }
       }
       ...
       for(int j=0; j<gc.size;j++){
           ...
           if(j<gc.noofresources)
			...
			///System.out.println("     Best Y: " + gBestLocation.getLoc()[1]);
                            ...
                            System.out.print(" "+gc.LUFL.get(j-gc.noofresources).get(gc.gblocation.get(i).getLoc()[j]));
                        }
                       }
       System.out.println("");
       }
      ...
      System.out.println("ALL");
       ...
       for(int j=0; j<gc.size-1;j++){
			...
			///System.out.println("     Best Y: " + gBestLocation.getLoc()[1]);
                        if(j==(gc.size-2)){
                            //output=output+"     Best R"+(j+1)+": "+gc.UFList.get(gBestLocation.getLoc()[j+1]);
                            System.out.print(" "+gc.UFList.get(gc.gblocation.get(i).getLoc()[j+1]));
                        }
                       }
       System.out.println("");
       }
        ...
        DecimalFormat df = new DecimalFormat("#.##");
            if (input == power) {
                String text = power.getText();
                 ...
                 double minpower=0.0;
        if(gc.nestedloop){
                maxpower=psn.calculatePower(max_component_detail);
                minpower=psn.calculatePower(min_component_detail);
            }
               else  if(gc.CDFG){
                maxpower=ps.calculatePower(max_component_detail);
                minpower=ps.calculatePower(min_component_detail);
            }
        else{
            ...
            minpower=psdfg.calculatePower(min_component_detail);
                ...
                double minTexe=0.0;
               if(gc.nestedloop){
                    maxTexe=psn.calculateFinalTex(min_component_detail);
                    System.out.println("max Texe=  "+maxTexe);
                    minTexe=psn.calculateFinalTex(max_component_detail);
                    System.out.println("min Texe=  "+minTexe);
                }
               else if(gc.CDFG){
                    maxTexe=ps.calculateTex(min_component_detail);
                    minTexe=ps.calculateTex(max_component_detail);
                }
                else{
               ...
               double value = Double.parseDouble(text);
                ...
                double minarea=0.0;
                if(gc.nestedloop){
                maxarea=psn.calculateArea(max_component_detail);
                minarea=psn.calculateArea(min_component_detail);
                }
               else if(gc.CDFG){
                maxarea=ps.calculateArea(max_component_detail);
                minarea=ps.calculateArea(min_component_detail);
                }
                else{
